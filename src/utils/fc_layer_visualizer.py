...
class VisualizerCallback(tflearn.callbacks.Callback):
    def __init__(self, model, x, output_dir,
                 layers_to_observe=(),
                 inputs=3):
        self.outdir = output_dir.rstrip('/') + '/'
        self.model = model
        self.x = x
        self.inputs = inputs
    ...
    def on_epoch_end(self, training_state):
        print ("fc_observers:", self.observers)
        ...
        for idx in range(self.inputs):
            ...
            for output in outputs:
                ...
                plt.subplot(len(outputs), 1, ix)
                plt.imshow(output[idx, :].reshape([1, -1]))
                plt.axis('off')
                ix += 1
                ...
                #    plt.axis('off')
                #    ix += 1
            plt.savefig(self.outdir + 'fc_outputs-for-image:%i-at-epoch:%i.png'
