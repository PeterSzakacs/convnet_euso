...
def get_train_and_test_sets(dataset, test_num_items=None, test_fraction=0.1):
    if test_fraction > 1:
        raise ValueError(('Requested an evaluation set from original dataset,'
                          ...
                          test_fraction*100))
    if test_num_items is not None and test_num_items > dataset.size:
    ...
    test_num = test_num_items or round(test_fraction * dataset.num_data)
    train_data = dataset.get_data_as_dict(slice(test_num))
    train_targets = dataset.get_targets(slice(test_num))

    num_data = dataset.num_data
    test_data = dataset.get_data_as_dict(slice(test_num, num_data))
    test_targets = dataset.get_targets(slice(test_num, num_data))
